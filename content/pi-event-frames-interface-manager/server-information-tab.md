...
The PI Data server specifies the server to which the interface sends PI tag data. _Host_ is the IP address or fully-qualified domain name of the PI Data server. If the server that you want to use is not listed in the drop-down menu, add it to the known servers table using the [PI System Explorer](https://docs.aveva.com/bundle/pi-server-af-pse/page/1019495.html).
